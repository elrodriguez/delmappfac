<?php

namespace App\Http\Livewire\Logistics\Production;

use App\Models\Logistics\Production\Project;
use App\Models\Logistics\Production\ProjectEmployees;
use App\Models\Logistics\Production\ProjectMaterial;
use App\Models\Logistics\Production\Stage;
use App\Models\Warehouse\Inventory;
use App\Models\Warehouse\InventoryKardex;
use App\Models\Warehouse\InventoryTransaction;
use App\Models\Warehouse\Item;
use Carbon\Carbon;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Lang;
use Livewire\Component;

class ProjectMaterialsForm extends Component
{
    public $description;
    public $materials = [];
    public $project_id;
    public $project_description;
    public $stages = [];
    public $stage_id;
    public $item_id;
    public $project;
    public $warehouse_id = 1;

    public function mount($project_id){
        $this->project_id = $project_id;
        $this->project = Project::find($project_id);
        $this->project_description = $this->project->description;
    }

    public function render()
    {

        $this->materialsList();
        $this->stages = Stage::where('project_id',$this->project_id)->get();
        return view('livewire.logistics.production.project-materials-form');
    }

    public function store(){

        $this->validate([
            'stage_id' => 'required',
            'item_id' => 'required',
        ]);

        $item = Item::where('id',$this->item_id)->first();

        ProjectMaterial::create([
            'item_id' => $item->id,
            'project_id' => $this->project_id,
            'stage_id' => $this->stage_id,
            'quantity' => 1,
            'unit_price' => $item->purchase_unit_price,
            'expenses' => ($item->purchase_unit_price*1),
            'brand_id' => $item->id_brand
        ]);

        $this->item_id = null;
        $this->stage_id = null;
        $this->dispatchBrowserEvent('response_projects_materials', ['message' => Lang::get('messages.successfully_registered')]);
    }

    public function materialsList(){
        $materials = ProjectMaterial::join('items','project_materials.item_id','items.id')
                ->join('unit_types','items.unit_type_id','unit_types.id')
                ->where('project_id',$this->project_id)
                ->select(
                    'project_materials.id',
                    'project_materials.quantity',
                    'project_materials.unit_price',
                    'project_materials.expenses',
                    'items.description',
                    'items.id AS item_id',
                    'items.module_type',
                    'project_materials.obsolete_quantity',
                    'project_materials.lost_quantity',
                    'project_materials.pending_quantity',
                    'project_materials.leftovers_quantity',
                    'project_materials.observations',
                    'project_materials.state',
                    'unit_types.description AS measure',
                    DB::raw("(SELECT name FROM brands WHERE brands.id=project_materials.brand_id) as name")
                )->get();

        foreach($materials as $key => $material){
            $file = 'storage/items/'.$material->id.'.jpg';
            $image = '';
            if(file_exists(public_path($file))){
                $image = asset('storage/items/'.$material->id.'.jpg');
            }else{
                $image = ui_avatars_url($material->description,50,'none',0);
            }
            $this->materials[$key] = [
                'id' => $material->id,
                'quantity' => $material->quantity,
                'unit_price' => $material->unit_price,
                'expenses' => $material->expenses,
                'description' => $material->description,
                'item_id' => $material->item_id,
                'module_type' => $material->module_type,
                'obsolete_quantity' => $material->obsolete_quantity,
                'lost_quantity' => $material->lost_quantity,
                'pending_quantity' => $material->pending_quantity,
                'leftovers_quantity' => $material->leftovers_quantity,
                'observations' => $material->observations,
                'state' => $material->state,
                'measure' => $material->measure,
                'name' => $material->name,
                'image_url' => $image
            ];
        }
        //dd($this->materials);
    }

    public function deleteMateiral($id){
        ProjectMaterial::where('id',$id)->delete();
    }
    public function orderMaterials(){
        $total = ProjectEmployees::where('project_id',$this->project_id)->sum('salary');


        foreach($this->materials as $item){
            if($item['state'] == 0){
                ProjectMaterial::where('id',$item['id'])->update([
                    'state' => 1
                ]);

            }
            $total = $total+$item['expenses'];
        }
        $this->project->update([
            'investment' => $total
        ]);
    }

    public function returnMaterials(){

        $inventory_transaction_tool = InventoryTransaction::where('id','29')->first();
        $inventory_transaction_lost = InventoryTransaction::where('id','14')->first();
        $inventory_transaction_obsolete = InventoryTransaction::where('id','15')->first();
        $inventory_transaction_leftover = InventoryTransaction::where('id','20')->first();

        foreach($this->materials as $item){

            if($item['state'] <> 5){
                $obsolete = $item['obsolete_quantity'];
                $lost = $item['lost_quantity'];
                $pending = $item['pending_quantity'];
                $leftover = $item['leftovers_quantity'];
                $total = $item['quantity'];

                $return = $total - ($lost+$obsolete);

                if($item['module_type'] == 'GOO'){
                    if($pending == 0){
                        $inventory = Inventory::create([
                            'description' => $inventory_transaction_tool->description,
                            'item_id' => $item['item_id'],
                            'quantity' => $return,
                            'warehouse_id' => $this->warehouse_id,
                            'inventory_transaction_id' => $inventory_transaction_tool->id
                        ]);

                        InventoryKardex::create([
                            'date_of_issue' => Carbon::now()->format('Y-m-d'),
                            'item_id' => $item['item_id'],
                            'inventory_kardexable_id' => $inventory->id,
                            'inventory_kardexable_type' => Inventory::class,
                            'warehouse_id' => $this->warehouse_id,
                            'quantity'=> $return
                        ]);
                        if($lost>0){
                            $inventory = Inventory::create([
                                'description' => $inventory_transaction_lost->description,
                                'item_id' => $item['item_id'],
                                'quantity' => $lost,
                                'warehouse_id' => $this->warehouse_id,
                                'inventory_transaction_id' => $inventory_transaction_lost->id
                            ]);
                            InventoryKardex::create([
                                'date_of_issue' => Carbon::now()->format('Y-m-d'),
                                'item_id' => $item['item_id'],
                                'inventory_kardexable_id' => $inventory->id,
                                'inventory_kardexable_type' => Inventory::class,
                                'warehouse_id' => $this->warehouse_id,
                                'quantity'=> (-$lost)
                            ]);
                        }
                        if($obsolete>0){
                            $inventory = Inventory::create([
                                'description' => $inventory_transaction_obsolete->description,
                                'item_id' => $item['item_id'],
                                'quantity' => $obsolete,
                                'warehouse_id' => $this->warehouse_id,
                                'inventory_transaction_id' => $inventory_transaction_obsolete->id
                            ]);
                            InventoryKardex::create([
                                'date_of_issue' => Carbon::now()->format('Y-m-d'),
                                'item_id' => $item['item_id'],
                                'inventory_kardexable_id' => $inventory->id,
                                'inventory_kardexable_type' => Inventory::class,
                                'warehouse_id' => $this->warehouse_id,
                                'quantity'=> (-$obsolete)
                            ]);
                        }
                        ProjectMaterial::where('id',$item['id'])->update([
                             'state' => 5
                        ]);
                    }
                }else{
                    if($leftover>0){
                        $inventory = Inventory::create([
                            'description' => $inventory_transaction_leftover->description,
                            'item_id' => $item['item_id'],
                            'quantity' => $leftover,
                            'warehouse_id' => $this->warehouse_id,
                            'inventory_transaction_id' => $inventory_transaction_leftover->id
                        ]);
                        InventoryKardex::create([
                            'date_of_issue' => Carbon::now()->format('Y-m-d'),
                            'item_id' => $item['item_id'],
                            'inventory_kardexable_id' => $inventory->id,
                            'inventory_kardexable_type' => Inventory::class,
                            'warehouse_id' => $this->warehouse_id,
                            'quantity'=> $leftover
                        ]);
                    }

                    ProjectMaterial::where('id',$item['id'])->update([
                         'state' => 5
                    ]);
                }
            }
        }
        $this->dispatchBrowserEvent('response_projects_materials', ['message' => Lang::get('messages.successfully_registered')]);
    }
}
