...
class IncomeEditForm extends Component
{
    ...
    public function mount($id_income){

        $this->purchase_id = $id_income;

        $purchase_items = PurchaseItem::join('purchases','purchase_items.purchase_id','purchases.id')
            ->where('purchases.id',$id_income)
            ->select(
                'purchase_items.item_id',
                DB::raw('JSON_EXTRACT(purchase_items.item, "$.item_code") AS purchase_item_item_code'),
                'purchase_items.quantity',
                DB::raw('JSON_EXTRACT(purchase_items.item, "$.description") AS purchase_item_description'),
                'purchases.*',
                DB::raw('JSON_EXTRACT(purchases.supplier, "$.name") AS purchase_supplier_name')
            )->get();
        $supplier_name = null;
        foreach($purchase_items as $key => $purchase_item){
            $this->purchase_items[$key] = [
                'id' => $purchase_item->item_id,
                'item_code' => $purchase_item->purchase_item_item_code,
                'description' => $purchase_item->purchase_item_description,
            ...
            $this->supplier_name = string_sanitize($purchase_item->purchase_supplier_name);
        }
    }
    ...
    public function render()
    {
        ...
        return view('livewire.warehouse.income-edit-form',['document_types'=>$document_types,'items'=>$items,'measurements'=>$measurements,'brands'=>$brands]);
    }
    ...
    public function updateDocument(){
        ...
        if($this->purchase_items){
            if($this->supplier_id){
                ...
                InventoryKardex::where('inventory_kardexable_id',$this->purchase_id)
