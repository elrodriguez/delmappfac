<?php

namespace App\Http\Livewire\Master;

use App\Models\Catalogue\Bank;
use App\Models\Catalogue\CurrencyType;
use App\Models\Catalogue\BankAccount;
use Elrod\UserActivity\Activity;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Lang;
use Livewire\Component;

class BankAccountEdit extends Component
{
    public $bank_id;
    public $description;
    public $currency_type_id;
    public $initial_balance;
    public $cci;
    public $number;
    public $state = true;
    public $banks;
    public $currency_types;
    public $bank_account_id;
    public $bank_account;

    public function mount($bank_account_id){

        $this->banks = Bank::all();
        $this->currency_types = CurrencyType::all();

        $this->bank_account_id = $bank_account_id;
        $this->bank_account = BankAccount::find($bank_account_id);

        $this->bank_id = $this->bank_account->bank_id;
        $this->description = $this->bank_account->description;
        $this->currency_type_id = $this->bank_account->currency_type_id;
        $this->initial_balance = $this->bank_account->initial_balance;
        $this->cci = $this->bank_account->cci;
        $this->number = $this->bank_account->number;
        $this->state = $this->bank_account->state;

    }

    public function render()
    {
        return view('livewire.master.bank-account-edit');
    }

    public function update(){

        $this->validate([
            'bank_id' => 'required',
            'description' => 'required|max:255',
            'currency_type_id' => 'required',
            'cci' => 'required',
            'number' => 'required'
        ]);

        $userActivity = new Activity;
        $userActivity->dataOld(BankAccount::find($this->bank_account_id));

        $this->bank_account->update([
            'bank_id' => $this->bank_id,
            'description' => $this->description,
            'number' => $this->number,
            'currency_type_id' => $this->currency_type_id,
            'cci' => $this->cci,
            'state' => $this->state,
            'initial_balance' => ($this->initial_balance?$this->initial_balance:0) 
        ]);

        
        $userActivity->causedBy(Auth::user());
        $userActivity->routeOn(route('configurations_master_bank_account_create'));
        $userActivity->componentOn('master.bank-account-create');
        $userActivity->log(Lang::get('messages.was_successfully_updated'));
        $userActivity->dataUpdated($this->bank_account);
        $userActivity->logType('create');
        $userActivity->modelOn(BankAccount::class,$this->bank_account_id);
        $userActivity->save();
        
        $this->dispatchBrowserEvent('response_bank_account_update', ['message' => Lang::get('messages.was_successfully_updated')]);
    }
}
