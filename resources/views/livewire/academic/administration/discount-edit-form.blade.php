<div class="panel-container show">
    <form class="needs-validation {{ ($errors->any()?'was-validated':'') }}" novalidate wire:ignore.self wire:submit.prevent="update()">
        <div class="panel-content">
            ...
            <div class="form-row">
                <div class="col-md-3 mb-3">
                    <label class="form-label" for="percentage">@lang('messages.percentage') <span class="text-danger">*</span> </label>
                    <input type="text" class="form-control" id="percentage" name="percentage" wire:model="percentage">
                    @error('percentage')
                    <div class="invalid-feedback-2">{{ $message }}</div>
                    @enderror
                </div>
        ...
        <div class="panel-content border-faded border-left-0 border-right-0 border-bottom-0 d-flex flex-row align-items-center">
            <a href="{{ route('academic_discounts') }}" type="button" class="btn btn-secondary waves-effect waves-themed"><i class="fal fa-long-arrow-left mr-1"></i>@lang('messages.back')</a>
    ...
    <script>
        window.addEventListener('response_success_discount', event => {
