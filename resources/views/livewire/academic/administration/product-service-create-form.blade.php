<div class="panel-container show">
    <form class="needs-validation {{ ($errors->any()?'was-validated':'') }}" novalidate wire:ignore.self wire:submit.prevent="store()">
        <div class="panel-content">
            ...
            <div class="form-row">
                ...
                @if($item_type_id=='01')
                <div class="col-md-3 mb-3">
                    <label class="form-label">{{ __('messages.stock') }} <span class="text-danger">*</span></label>
                    <input type="text" class="form-control" name="stock" wire:model.defer="stock" required="">
                    @error('stock')
                        <div class="invalid-feedback-2">{{ $message }}</div>
                    @enderror
                </div>
    ...
    <script>
        window.addEventListener('response_success_product_service', event => {
