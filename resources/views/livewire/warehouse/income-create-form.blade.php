<div>
    <form class="needs-validation" wire:ignore.self novalidate wire:submit.prevent="saveDocument()">
        <div class="panel-content">
            <div class="form-row">
                <div class="col-md-4 mb-3">
                    <label class="form-label">Tipo comprobante <span class="text-danger">*</span> </label>
                    <select class="form-control" wire:model="document_type" name="document_type">
                        ...
                        <tbody>
                            @foreach ($purchase_items as $key => $item)
                                <tr>
                                    ...
                                    <td class="text-center"><span class="badge border border-dark text-dark">{{ $item['item_code'] }}</span></td>
                                    <td>{{ $item['description'] }}</td>
                                    <td class="text-center"><input type="number" wire:model.defer="purchase_items.{{ $key }}.quantity" class="form-control text-right"></td>
        ...
        <div class="panel-content border-faded border-left-0 border-right-0 border-bottom-0 d-flex flex-row align-items-center">
            ...
            <button class="btn btn-primary ml-auto waves-effect waves-themed" wire:loading wire:target="saveDocument" wire:loading.attr="disabled" >
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <div class="input-group">
                        <input id="item_code" type="text" name="item_code" class="form-control" wire:model.defer="item_code" @if($checkmeout2) {{ 'disabled' }} @endif>
                        <div class="input-group-append">
                ...
                <div id="js_default_list" class="js-list-filter">
                    ...
                    @if($items)
                        <ul class="list-group">
                            ...
                            <li class="list-group-item list-group-item-action" style="cursor: pointer" wire:click="addProductBox('{{ $row->id }}','{{ $row->item_code }}','{{ $row->name }} - {{ $row->description }}')">
                                <i class="fal fa-check-circle width-2 fs-xl"></i>
                                <span class="badge border border-dark text-dark">{{ $row->item_code }} </span>
