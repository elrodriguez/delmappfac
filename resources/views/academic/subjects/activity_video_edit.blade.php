<x-app-layout>
    ...
    <x-slot name="header">
        <ol class="breadcrumb page-breadcrumb">
            ...
            <li class="breadcrumb-item active">@lang('messages.video')</li>
            <li class="position-absolute pos-top pos-right d-none d-sm-block">@livewire('js-get-date')</li>
        </ol>
        <div class="subheader">
            <h1 class="subheader-title">
                <i class="fal fa-video-plus"></i> @lang('messages.video')
            </h1>
        </div>
    </x-slot>
    <div class="row">
        <div class="col-xl-12">
            <div id="panel-2" class="panel">
                <div class="panel-hdr">	
                    <h2>
                        Subir  <span class="fw-300"><i>Video</i></span> 
                    </h2>
                ...
                <div class="panel-container show">
                    <form id="formVideo" action="{{ route('academic_subjects_student_video_vimeo_store') }}" method="POST" enctype="multipart/form-data">
                        <div class="panel-content">
                            ...
                            <div class="form-row">
                                <div class="col-md-12 mb-3">
									<label class="form-label" for="description">{{ __('messages.aca_video_description') }} <span class="text-danger">*</span> </label>
									<textarea name="description" id="description" class="form-control" >{{ $activity->body }}</textarea>
									@error('description') <span class="invalid-feedback-2">{{ $message }}</span> @enderror
								</div>
                            </div>
                            <div class="form-row">
								<div class="col-md-12 mb-3">
									<label class="form-label" for="video">{{ __('messages.aca_video_file') }} <span class="text-danger">*</span> </label><br>
									<input type="file" name="video" id="video" ><br>
									@error('video') <span class="invalid-feedback-2">{{ $message }}</span> @enderror
								</div>
                            </div>
                            <div class="form-row">
                                <div class="col-md-12 mb-3">
                        ...
                        <div class="panel-content border-faded border-left-0 border-right-0 border-bottom-0 d-flex flex-row align-items-center">
                            ...
                            <button style="display: none" id="btnProgressVideo" class="btn btn-primary ml-auto waves-effect waves-themed"" type="button" disabled="">
    ...
    <!-- Modal center Small -->
    <div class="modal fade" id="progressVideo" tabindex="-1" role="dialog" aria-hidden="true">
    ...
    <script type="text/javascript">
        $(function () {
            $(document).ready(function () {
                ...
                $('#formVideo').ajaxForm({
                    beforeSend: function () {
                        $('#progressVideo').modal('show');
                        $('#labelMsgVideo').html("{{ __('messages.aca_loading') }}...");
                        $('#progressVideo').modal({backdrop: 'static', keyboard: false});
                        $("#btnProgressVideo").css('display','block');
                        $("#btnUpdateVideo").css('display','none');
                    },
                    uploadProgress: function (event, position, total, percentComplete) {
                        //$('#labelMsgVideo').html("{{ __('messages.aca_ending') }}")
                    },
                    complete: function (xhr) {
                        $('#progressVideo').modal('hide');

                        $("#btnCloseVideo").removeAttr('disabled');
                        $("#btnUpdateVideo").css('display','block');
                        $("#btnProgressVideo").css('display','none');
                        Swal.fire("{{ __('messages.congratulations') }}","{{ __('messages.file_has_been_uploaded_successfully') }}", "info");
                    }
                });
            });
        });
