<x-app-layout>
    ...
    <x-slot name="header">
        ...
        <div class="subheader">
            <h1 class="subheader-title">
                <i class='fal fa-info-circle'></i> {{ __('Usarios') }}
            </h1>
        </div>
    </x-slot>
    <div class="row">
    ...
    <script defer>
                            ...
                            render: function(data){
                                var buttons = `<div class="dropdown">
                                                        ...
                                                        <div class="dropdown-menu" style="">
                                                            ...
                                                            <a class="dropdown-item" href="`+data.roles_url+`"><i class="fal fa-lock-open-alt mr-1"></i>Asignar roles</a>
                                                            <a class="dropdown-item text-danger" href="javascript:void(0)" onclick="userdelete('`+data.delete_url+`')"><i class="fal fa-trash-alt mr-1"></i>Eliminar</a>
                                                        </div>
                                                    </div>`;
                                return buttons;
                            }
                    ...
                    buttons: [
                        {
                            ...
                            action:function(){
                                window.location.href = "{{ route('user_created') }}";
                            }
                        },
                        {
                            extend: 'excelHtml5',
            ...
            function userdelete(delete_url){
                Swal.fire(
                {
                    title: "¿Estás seguro?",
                    ...
                    if (result.value)
                    {
                        $.get(delete_url, function(data) {
                            if(data.success == true){
                                Swal.fire("¡Eliminado!", data.name+" ha sido eliminado.", "success");
